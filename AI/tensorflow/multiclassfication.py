...
w = tf.compat.v1.get_variable("weight", shape=[
                              2, 3], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=0.1))
bais = tf.compat.v1.get_variable(
    "bais", shape=[3], dtype=tf.float32, initializer=tf.constant_initializer(0))
y_1 = tf.nn.bias_add(tf.matmul(x, w), bais)

loss = tf.reduce_mean(
    tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=y_1))
x0min, x0max = X[:, 0].min(), X[:, 0].max()
x1min, x1max = X[:, 1].min(), X[:, 1].max()

with tf.compat.v1.Session() as sess:
    accuracy = tf.reduce_mean(
        tf.cast(tf.equal(tf.arg_max(y, 1), tf.arg_max(y_1, 1)), tf.float32))
    ...
    for i in range(3001):
        ...
        if i % 500 == 0:
            accuracy_print = sess.run(
                accuracy, feed_dict={x: X, y: iris_target1})
            print(accuracy_print)

    h = 0.05
    xx, yy = np.meshgrid(np.arange(x0min-1, x0max+1, h),
                         np.arange(x1min-1, x1max+1, h))
